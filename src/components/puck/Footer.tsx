...
// Social Links Component - renders as non-clickable icons (placeholders for real social URLs)
function SocialLinks({ styles, themePreset }: { styles: { link: string }; themePreset: string }) {
  return (
    <div className={cn("flex gap-4", themePreset === "zara" && "gap-6")}>
      <span className={cn("transition-colors cursor-default", styles.link)} aria-label="Twitter">
      ...
      <span className={cn("transition-colors cursor-default", styles.link)} aria-label="Instagram">
        <svg className={cn("w-6 h-6", themePreset === "zara" && "w-5 h-5")} fill="currentColor" viewBox="0 0 24 24">
